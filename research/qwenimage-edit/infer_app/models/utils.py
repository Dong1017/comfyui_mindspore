"""
Utilities for QwenImage-Edit inference.

Note: Only `_scaled_dot_product_attention_native` is needed here.
Other utility functions (calculate_shift, retrieve_timesteps, pack_latents, etc.)
are handled by the MindOne pipeline/scheduler classes.
"""
...
def _scaled_dot_product_attention_native(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, dtype=None):
    """
    Native attention implementation for QwenImage-Edit.

    This is used by the transformer models for custom attention logic.
    """
