...
from mindone.diffusers.loaders import FromOriginalModelMixin, PeftAdapterMixin
from mindone.diffusers.models._modeling_parallel import ContextParallelInput, ContextParallelOutput
from mindone.diffusers.models.attention import AttentionMixin, FeedForward
from mindone.diffusers.models.attention_processor import Attention
from mindone.diffusers.models.cache_utils import CacheMixin
...
def apply_rotary_emb_qwen(
    ...
    Args:
        x (`ms.Tensor`):
            Query or key tensor to apply rotary embeddings. [B, S, H, D] xk (ms.Tensor): Key tensor to apply
...
class QwenDoubleStreamAttnProcessor2_0:
    ...
    def __call__(
        self,
        attn: Attention,
        hidden_states: ms.Tensor,  # Image stream
        encoder_hidden_states: ms.Tensor = None,  # Text stream
        ...
        # Order: [text, image]
        joint_query = mint.cat([txt_query, img_query], dim=1)
        joint_key = mint.cat([txt_key, img_key], dim=1)
...
class QwenImageTransformerBlock(nn.Cell):
    ...
    def construct(
        ...
        encoder_hidden_states_mask: ms.Tensor,
        temb: ms.Tensor,
