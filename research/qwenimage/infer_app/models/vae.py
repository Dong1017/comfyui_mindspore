...
class QwenImageCausalConv2dFromConv3d(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        stride: Union[int, Tuple[int, int, int]] = 1,
        padding: Union[int, Tuple[int, int, int]] = 0,
        weight_conv3d: Optional[ms.Tensor] = None,
        bias_conv3d: Optional[ms.Tensor] = None,
    ) -> None:
        super().__init__()
        ...
        self.kernel_size = triple_sequence(kernel_size)
        self.stride = triple_sequence(stride)
        padding = triple_sequence(padding)

        self.bias = bias_conv3d
        self.time_stride = self.stride[0]  # no use, cuz decoder always set 1 for stride
        self._padding = (padding[2], padding[2], padding[1], padding[1], 2 * padding[0], 0)

        # weight_conv3d -> weight_conv2d_list
        self.weight_conv2d_list = []
        for t in range(weight_conv3d.shape[2]):
            w = weight_conv3d[:, :, t, :, :]  # [C_out, C_in/groups, D, H, W]
            self.weight_conv2d_list.append(w)
    ...
    def construct(self, x, cache_x=None):
        ...
        for i in range(n):
            if i == 0:
                conv_out = conv2d_ext_op(
                    x[:, :, i, :, :],  # [B, C_in, H, W]
                    self.weight_conv2d_list[i],
                    self.bias,
                    self.stride[1:3],
                    0,  # self.padding
                    self.dilation,
            ...
            outputs.append(conv_out.unsqueeze(2))
        result = mint.sum(mint.stack(outputs, dim=2), dim=2)
    ...
    def construct(self, x, feat_cache=None, feat_idx=[0]):
        ...
        if self.mode == "upsample3d":
            _time_conv_native = QwenImageCausalConv2dFromConv3d(
                ...
                (3, 1, 1),
                padding=(1, 0, 0),
                weight_conv3d=self.time_conv.weight,
                bias_conv3d=self.time_conv.bias,
            )
            if feat_cache is not None:
                ...
                if feat_cache[idx] is None:
                    ...
                    if feat_cache[idx] == "Rep":
                        x = _time_conv_native(x)
                    else:
                        x = _time_conv_native(x, feat_cache[idx])
...
class QwenImageResidualBlock(nn.Cell):
    ...
    def construct(self, x, feat_cache=None, feat_idx=[0]):
        _conv1_native = QwenImageCausalConv2dFromConv3d(
            self.in_dim,
            self.out_dim,
            3,
            padding=1,
            weight_conv3d=self.conv1.weight,
            bias_conv3d=self.conv1.bias,
        )
        _conv2_native = QwenImageCausalConv2dFromConv3d(
            self.out_dim,
            self.out_dim,
            3,
            padding=1,
            weight_conv3d=self.conv2.weight,
            bias_conv3d=self.conv2.bias,
        )
        _conv_shortcut_native = (
            QwenImageCausalConv2dFromConv3d(
                self.in_dim,
                self.out_dim,
                1,
                weight_conv3d=self.conv_shortcut.weight,
                bias_conv3d=self.conv_shortcut.bias,
            )
        ...
        if feat_cache is not None:
            ...
            x = _conv1_native(x, feat_cache[idx])
            feat_cache[idx] = cache_x
            feat_idx[0] += 1
        else:
            x = _conv1_native(x)
        ...
        if feat_cache is not None:
            ...
            x = _conv2_native(x, feat_cache[idx])
            feat_cache[idx] = cache_x
            feat_idx[0] += 1
        else:
            x = _conv2_native(x)
...
class QwenImageDecoder3d(nn.Cell):
    ...
    def construct(self, x, feat_cache=None, feat_idx=[0]):
        ...
        _conv_in_native = QwenImageCausalConv2dFromConv3d(
            self.z_dim,
            dims[0],
            3,
            padding=1,
            weight_conv3d=self.conv_in.weight,
            bias_conv3d=self.conv_in.bias,
        )
        if feat_cache is not None:
            idx = feat_idx[0]
            cache_x = x[:, :, -CACHE_T:, :, :].clone()
            if cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                # cache last frame of last two chunk
                cache_x = mint.cat([feat_cache[idx][:, :, -1, :, :].unsqueeze(2), cache_x], dim=2)
            x = _conv_in_native(x, feat_cache[idx])
            feat_cache[idx] = cache_x
            feat_idx[0] += 1
        else:
            x = _conv_in_native(x)
        ...
        x = self.norm_out(x)
        x = self.nonlinearity(x)
        _conv_out_native = QwenImageCausalConv2dFromConv3d(
            dims[1:][-1],
            3,
            3,
            padding=1,
            weight_conv3d=self.conv_out.weight,
            bias_conv3d=self.conv_out.bias,
        )
        if feat_cache is not None:
            idx = feat_idx[0]
            cache_x = x[:, :, -CACHE_T:, :, :].clone()
            if cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                # cache last frame of last two chunk
                cache_x = mint.cat([feat_cache[idx][:, :, -1, :, :].unsqueeze(2), cache_x], dim=2)
            x = _conv_out_native(x, feat_cache[idx])
            feat_cache[idx] = cache_x
            feat_idx[0] += 1
        else:
            x = _conv_out_native(x)
        return x
...
class AutoencoderKLQwenImage(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def _decode(self, z: ms.Tensor, return_dict: bool = False):
        ...
        self.clear_cache()
        _post_quant_conv_native = QwenImageCausalConv2dFromConv3d(
            self.z_dim,
            self.z_dim,
            1,
            weight_conv3d=self.post_quant_conv.weight,
            bias_conv3d=self.post_quant_conv.bias,
        )
        x = _post_quant_conv_native(z)
